...
export default async function ReceiptPage({ params }: { params: { id: string } }) {
  ...
  const { data: request } = await supabase
    ...
    .select(`
      *,
      customer:customers(customer_name, imei, mobile, model_no, aadhaar, first_emi_charge_amount, retailer:retailers(name)),
  ...
  const retailer = request.retailer as { name?: string; username?: string } | null;
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #fefce8 0%, #f0f9ff 60%, #f8fafc 100%)', padding: '2rem 1rem', fontFamily: 'DM Sans, system-ui, sans-serif' }}>
      <div style={{ maxWidth: '480px', margin: '0 auto' }}>
        ...
        <div id="no-print" style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center', marginBottom: '1.5rem' }}>
          <PrintButton />
          <button
            onClick={() => {}}
            style={{ padding: '0.625rem 1.25rem', background: '#25d366', color: 'white', border: 'none', borderRadius: '0.75rem', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem' }}
          >
            Share via WhatsApp
          </button>
        </div>
        ...
        <div style={{ background: 'white', borderRadius: '1.5rem', overflow: 'hidden', boxShadow: '0 8px 40px rgba(0,0,0,0.12)', border: '1px solid #e2e8f0' }}>
          ...
          <div style={{ padding: '1.5rem 2rem' }}>
            ...
            <Section title="TRANSACTION DETAILS">
              ...
              {retailer?.username && (
                <KV label="Retailer ID" value={`@${retailer.username}`} mono small />
              )}
            </Section>
